...
func (d *AgentDeployer) DeployAgent(ctx context.Context, cellAgent config.CellAgent) error {
	return d.DeployAgentWithEnv(ctx, cellAgent, nil)
}

// DeployAgentWithEnv deploys an agent with custom environment variables
// This allows the embedded orchestrator to inject project-specific settings
// like GOX_DATA_ROOT and GOX_PROJECT_ID for VFS isolation
func (d *AgentDeployer) DeployAgentWithEnv(ctx context.Context, cellAgent config.CellAgent, customEnv map[string]string) error {
	...
	case "spawn":
		return d.spawnAgentWithEnv(ctx, cellAgent, agentTypeConfig, customEnv)
	case "call":
		return d.callAgentWithEnv(ctx, cellAgent, agentTypeConfig, customEnv)
	case "await":
...
func (d *AgentDeployer) spawnAgent(ctx context.Context, cellAgent config.CellAgent, typeConfig config.AgentTypeConfig) error {
	return d.spawnAgentWithEnv(ctx, cellAgent, typeConfig, nil)
}

// spawnAgentWithEnv starts an agent with custom environment variables
func (d *AgentDeployer) spawnAgentWithEnv(ctx context.Context, cellAgent config.CellAgent, typeConfig config.AgentTypeConfig, customEnv map[string]string) error {
...
func (d *AgentDeployer) callAgent(ctx context.Context, cellAgent config.CellAgent, typeConfig config.AgentTypeConfig) error {
	return d.callAgentWithEnv(ctx, cellAgent, typeConfig, nil)
}

// callAgentWithEnv executes an agent with custom environment variables
func (d *AgentDeployer) callAgentWithEnv(ctx context.Context, cellAgent config.CellAgent, typeConfig config.AgentTypeConfig, customEnv map[string]string) error {
	...
	return d.spawnAgentWithEnv(ctx, cellAgent, typeConfig, customEnv)
}
