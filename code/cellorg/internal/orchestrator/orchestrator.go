// Package orchestrator provides pipeline-level coordination for agent deployment
// and dependency management within the GOX framework.
//
// The orchestrator manages agent startup/shutdown sequences based on dependency
// graphs, ensuring that agents start in the correct order and their dependencies
// are satisfied before they begin processing.
//
// Key Responsibilities:
// - Dependency graph construction and circular dependency detection
// - Topological sort for startup order determination
// - Agent state monitoring and coordination
// - Graceful shutdown in reverse dependency order
//
// Called by: Agent deployer, pipeline management systems
// Calls: Support client for agent state queries, agent lifecycle signals
...
// These represent the lifecycle states that agents transition through
const (
	StateInstalled  agent.AgentState = "installed"  // Agent binary available but not configured
	StateConfigured agent.AgentState = "configured" // Agent configured but not started
	StateReady      agent.AgentState = "ready"      // Agent ready to process messages
	StateRunning    agent.AgentState = "running"    // Agent actively processing
	StatePaused     agent.AgentState = "paused"     // Agent temporarily suspended
	StateStopped    agent.AgentState = "stopped"    // Agent cleanly shut down
	StateError      agent.AgentState = "error"      // Agent in error state requiring intervention
)

// PipelineOrchestrator manages agent startup/shutdown with dependency resolution.
//
// The orchestrator builds dependency graphs from configuration and coordinates
// agent lifecycle events to ensure proper startup/shutdown ordering. It uses
// topological sorting for dependency resolution and monitors agent states
// through the support service.
//
// Thread Safety: All public methods are thread-safe through internal mutexes
type PipelineOrchestrator struct {
	supportClient *client.SupportClient // Connection to support service for agent state queries
	debug         bool                  // Enable debug logging

	// Dependency graph and state management
	dependencyGraph map[string][]string   // agentID -> list of dependencies (who this agent waits for)
	agents          map[string]*AgentNode // agentID -> agent metadata and runtime state
	agentsMux       sync.RWMutex          // Protects agents map for concurrent access

	// Orchestration state for startup/shutdown coordination
	startupOrder     []string   // Topologically sorted order for agent startup
	shutdownOrder    []string   // Reverse order for graceful shutdown
	orchestrationMux sync.Mutex // Protects orchestration state during resolution

	// Event channels for state change monitoring
	stateChanges    chan agent.StateChange // Channel for receiving agent state updates
	orchestratorCtx context.Context        // Context for orchestrator lifecycle
	cancel          context.CancelFunc     // Function to cancel orchestrator operations
}

// AgentNode represents an agent in the dependency graph with runtime state tracking.
//
// This structure maintains both the static configuration (dependencies, type)
// and dynamic runtime information (state, ping times, start attempts) for
// each agent in the pipeline.
type AgentNode struct {
	ID           string                 `json:"id"`           // Unique agent instance identifier
	AgentType    string                 `json:"agent_type"`   // Type from pool.yaml (e.g., "text-extractor")
	State        agent.AgentState       `json:"state"`        // Current lifecycle state
	Dependencies []string               `json:"dependencies"` // Agents this agent waits for
	Dependents   []string               `json:"dependents"`   // Agents that wait for this agent
	Config       map[string]interface{} `json:"config"`       // Agent-specific configuration
	LastPing     time.Time              `json:"last_ping"`    // Last health check timestamp

	// Runtime state tracking for orchestration
	startAttempts    int       // Number of start attempts for retry logic
	lastStartAttempt time.Time // Timestamp of last start attempt
	isStarting       bool      // True when agent is in startup process
	isStopping       bool      // True when agent is in shutdown process
}
...
//
// Establishes connection to the support service for agent state monitoring
// and initializes internal data structures for dependency management.
//
// Parameters:
//   - config: Orchestration configuration including support service address
//     and timeout settings
//
// Returns:
//   - *PipelineOrchestrator: Configured orchestrator ready for dependency loading
//   - error: Connection error if support service is unreachable
//
// Called by: Agent deployer during initialization
// Calls: client.NewSupportClient(), supportClient.Connect()
func NewPipelineOrchestrator(config OrchestrationConfig) (*PipelineOrchestrator, error) {
	// Establish connection to support service for agent state queries
	...
	// Create cancellation context for orchestrator lifecycle management
	ctx, cancel := context.WithCancel(context.Background())

	orchestrator := &PipelineOrchestrator{
		supportClient:   supportClient,
		debug:           config.Debug,
		dependencyGraph: make(map[string][]string),         // Initialize empty dependency graph
		agents:          make(map[string]*AgentNode),       // Initialize agent registry
		stateChanges:    make(chan agent.StateChange, 100), // Buffered channel for state events
		orchestratorCtx: ctx,
		cancel:          cancel,
	}
...
//
// Uses Kahn's algorithm to resolve a valid startup order that respects all
// dependency constraints. Agents with no dependencies start first, followed
// by agents whose dependencies have already started.
//
// Algorithm Steps:
// 1. Detect circular dependencies using DFS traversal
// 2. Calculate in-degrees (number of dependencies) for each agent
// 3. Process agents with zero in-degree (no dependencies)
// 4. Remove processed agents from dependency graph and update in-degrees
// 5. Repeat until all agents are processed or circular dependency detected
//
// Returns:
//   - nil: Successful resolution with startupOrder and shutdownOrder populated
//   - error: Circular dependency detected or other resolution failure
//
// Called by: LoadDependencies() after dependency graph construction
// Calls: detectCircularDependencies()
func (o *PipelineOrchestrator) resolveDependencyOrder() error {
	// First pass: detect circular dependencies using depth-first search
	if err := o.detectCircularDependencies(); err != nil {
		return err
	}

	// Kahn's algorithm for topological sort
	inDegree := make(map[string]int) // Count of dependencies for each agent
	queue := make([]string, 0)       // Agents ready to start (no pending dependencies)
	result := make([]string, 0)      // Final startup order
	...
	// Find nodes with no dependencies (in-degree 0) to start processing
	...
	// Process queue until empty (all dependencies resolved)
	for len(queue) > 0 {
		// Sort queue for deterministic ordering across runs
		sort.Strings(queue)
		current := queue[0]
		queue = queue[1:]
		result = append(result, current)

		// Update dependents: reduce their in-degree and add to queue if ready
	...
	// Verify all nodes were processed (no remaining circular dependencies)
	if len(result) != len(o.agents) {
		return fmt.Errorf("circular dependency detected - could not resolve all dependencies")
	}

	// Store resolved orders
	o.startupOrder = result
	// Shutdown order is reverse of startup order for graceful teardown
...
// StartCellWithEnv starts a cell with custom environment variables and VFS root.
// This method is used by the embedded orchestrator to start cells with project-specific
// configurations while preserving the standard Gox cell startup flow.
//
// Parameters:
//   - cellID: Cell identifier from cells.yaml (e.g., "rag:knowledge-backend")
//   - env: Environment variables to inject (including GOX_DATA_ROOT for VFS override)
//   - cellConfig: Cell configuration overrides
//
// Returns:
//   - error: Cell startup error or nil on success
//
// Called by: pkg/orchestrator.EmbeddedOrchestrator.StartCell()
func (o *PipelineOrchestrator) StartCellWithEnv(
	cellID string,
	...
	if o.debug {
		log.Printf("Starting cell %s with custom environment", cellID)
	}

	// TODO: Implement actual cell loading and agent deployment
	// This requires:
	// 1. Load cell configuration from cells.yaml by cellID
	// 2. For each agent in cell:
	//    - Apply environment overrides (esp. GOX_DATA_ROOT for VFS)
	//    - Apply configuration overrides
	//    - Start agent process
	// 3. Load agent dependencies and resolve startup order
	// 4. Monitor agent health through support service

	// Placeholder for Phase 2 implementation
	log.Printf("[Orchestrator] Cell startup placeholder: cellID=%s, env=%v", cellID, env)
