...
func NewService(config interface{}) *Service {
	...
	// Agent types will be loaded by orchestrator via LoadAgentTypesFromFile
	// This prevents duplicate loading and allows proper path resolution
...
func (s *Service) validateAgentType(agentType string) bool {
	...
	// Allow test agent types in debug mode (for testing)
	if s.debug && (agentType == "test-support-agent" ||
...
func (s *Service) handleGetAgentCellConfig(req *Request) *Response {
	...
	agentMap, err := s.loadCellConfig("config/cells.yaml")
	if err != nil {
		return &Response{
			ID:    req.ID,
...
// For loading config/cells.yaml and looking up agent cell configs
...
// Loads config/cells.yaml and returns a map[agent id]AgentCellConfig
...
func (s *Service) handleGetCellOrchestrationConfig(req *Request) *Response {
	...
	cellConfigs, err := s.loadCellConfigs("config/cells.yaml")
	if err != nil {
		return &Response{
			ID:    req.ID,
			Error: &Error{Code: -32603, Message: "Failed to load cell configs: " + err.Error()},
